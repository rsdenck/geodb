...
func runSchema() {
	...
	for _, h := range hypertablesSetup {
		if _, err := db.pool.Exec(ctx, h); err != nil {
			fmt.Fprintf(os.Stderr, "  ERRO hypertable: %v\n", err)
		}
	}
	fmt.Println("  Hypertables criadas")

	for _, idx := range indexesSetup {
		if _, err := db.pool.Exec(ctx, idx); err != nil {
			fmt.Fprintf(os.Stderr, "  ERRO index: %v\n", err)
		}
	}
	fmt.Println("  Indexes criados")
	fmt.Println("=== SCHEMA PRONTO ===")
}
